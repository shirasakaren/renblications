...
export function ProfileSettings({ initialProfile }: { initialProfile: AuthorProfile }) {
  ...
  function uploadAvatar(event: ChangeEvent<HTMLInputElement>) {
    ...
    body.set("file", file);
    body.set("alt", profile.name);
    setMessage("Uploading avatar");
    startTransition(async () => {
      const response = await fetch("/api/admin/media", { method: "POST", body });
      const result = (await response.json()) as { item?: { url: string }; error?: string };
      if (!response.ok || !result.item) {
        setMessage(result.error || "Avatar could not be uploaded.");
        return;
      }
      setProfile((current) => ({ ...current, avatarUrl: result.item?.url || current.avatarUrl }));
      setMessage("Avatar uploaded. Save the profile to publish it.");
    });
  }
